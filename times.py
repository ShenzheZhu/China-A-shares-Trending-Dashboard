...
# 支持: ETF代码(如"511260") 或 指数代码(如"sh000300")
SYMBOL = '511260'  # 国债ETF
SYMBOL_TYPE = 'etf'  # 'etf' 或 'index'
START_DATE = '20150101'
END_DATE = '20240101'
...
class DataEngine:
    ...
    def load(self):
        if os.path.exists(DATA_CACHE_PATH):
            ...
            print(f"🌐 Fetching {SYMBOL} ({SYMBOL_TYPE})...")
            ...
            if SYMBOL_TYPE == 'etf':
                ...
                df = df[(df['trade_date'] >= START_DATE) & (df['trade_date'] <= TEST_END_DATE)]

            if df is None or df.empty:
                raise ValueError("未获取到数据，请检查代码是否正确")
        ...
        print(f"✅ {SYMBOL} Data Ready. Shape: {self.feat_data.shape}")
        return self
...
def final_reality_check(miner, engine):
    ...
    plt.plot(test_dates, bench_equity, label=f'Benchmark ({SYMBOL})', alpha=0.5, linewidth=1)
