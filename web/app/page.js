      ...
      <div className="heatmap-section">
        ...
        <div className="heatmap-grid">
          {sectorAnalysis.map(sector => {
            ...
            const hue = maRatio * 120  // 0=红, 60=黄, 120=绿
            const lightness = 25 + maRatio * 20
            return (
              <div 
                ...
                style={{
                  ...
                  opacity: 0.7 + maRatio * 0.3
                }}
                title={`${sector.name}: ${maAbove}/${sector.count} 站上MA20`}
              >
                <span className="heatmap-name">{sector.name}</span>
                <span className="heatmap-ratio">{(maRatio * 100).toFixed(0)}%</span>
              </div>
            )
          })}
        </div>
          ...
          <div className="card" key={etf.code}>
            <div className="card-header">
              ...
              <div className="dual-signals">
                {etf.signalChange === 'breakthrough' && (
                  <span className="signal-change breakthrough">今日突破</span>
                )}
                {etf.signalChange === 'breakdown' && (
                  <span className="signal-change breakdown">今日跌破</span>
                )}
