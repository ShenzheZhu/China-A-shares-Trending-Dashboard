...
def generate_all_signals():
    ...
    for etf in ETF_LIST:
        ...
        try:
            ...
            # 信号变化检测：比较昨天和今天
            signal_change = 'none'
            if len(df) >= 2 and not np.isnan(df['MA20'].iloc[-2]):
                yesterday_price = df['close'].iloc[-2]
                yesterday_ma20 = df['MA20'].iloc[-2]
                yesterday_above = yesterday_price > yesterday_ma20
                
                if ma_above and not yesterday_above:
                    signal_change = 'breakthrough'  # 今日突破
                elif not ma_above and yesterday_above:
                    signal_change = 'breakdown'  # 今日跌破
